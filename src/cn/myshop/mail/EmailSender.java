...
public class EmailSender {
    ...
    public static void main(String[] args) throws Exception {
    	EmailSender.send(new String[]{"job@myshop.cn"}, "邮件测试xx", "<b>传智播客</b>", null , "text/html");
    }
    /**
     * 发送邮件
     * @param receiver 收件人
    ...
    public static void send(String[] receivers, String subject, String mailContent, File[] attachements, String mimetype) {
        ...
        props.put("mail.smtp.host", "mail.myshop.cn");//smtp服务器地址 sohu
        props.put("mail.smtp.auth", "true");//需要校验
        Session session = Session.getDefaultInstance(props, new Authenticator() {
            protected PasswordAuthentication getPasswordAuthentication() {
                return new PasswordAuthentication("job@myshop.cn","lihuoming");//登录用户名/密码
            }
        });
        session.setDebug(true);
        try {
            MimeMessage mimeMessage = new MimeMessage(session);
            mimeMessage.setFrom(new InternetAddress("job@myshop.cn"));//发件人邮件
