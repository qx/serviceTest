...
<title>${product.name}-famous</title>
...
<div id="ContentBody"><!-- 页面主体 -->
    <c:set var="out" value="&gt;&gt; <em>${product.name}</em>"/><c:forEach items="${stypes}" var="type" varStatus="statu">
		<c:set var="out" value=" &gt;&gt; <a href='/product/list.do?typeid=${type.typeid}'>${type.name}</a> ${out}"/>
	 	 ...
	 	 <div id="position"> 您现在的位置：<a href="/" name="linkHome">famous</a> <span id="uc_cat_spnPath"><c:out value="${out}" escapeXml="false"></c:out></span></div>
									...
									<ul>
										<li>市场价：<s>${product.marketprice}</s> 元 <font color='#ff6f02'>本站价：<b>${product.sellprice} 元</b></font> 节省：<font color='#ff6f02'>${product.savedPrice }</font> 元										</li>
									  	<li class="right_img"><INPUT TYPE="image" SRC="/images/global/sale.gif"></li>
																				<li class="guopiprice">
										[ <IMG src="/images/global/2j4.gif" border="0">&nbsp;<A href="http://localhost:8080/cache/news/6/9.shtml" target="_blank">配送说明</A> ]&nbsp;&nbsp;&nbsp;&nbsp;[ <IMG src="/images/global/2j4.gif" border="0">&nbsp;<A href="http://localhost:8080/cache/news/4/24.shtml" target="_blank">付款方式</A> ]
										</li>
									</ul>									
							</div>
</div>
