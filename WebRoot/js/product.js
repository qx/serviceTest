	function colorEvent(eventObject,productid){
		...
		if(eventObject && main_image){			
			...
			if(productimg && productimg.value.trim()!=""){
				main_image.style.background = "url(http://image.famous.com/global/init.gif) center center no-repeat";
				main_image.style.background = "url(http://image.famous.com/product/"+productid+"/200x/"+ productimg.value+") center center no-repeat";
			}			
		}
	}
